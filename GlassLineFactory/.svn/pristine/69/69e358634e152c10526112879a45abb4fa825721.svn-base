	...
	public void eventFired(TChannel channel, TEvent event, Object[] args)
	{
		...
		if (((Integer)args[0]).equals(index))
		{
			...
			if (event == TEvent.WORKSTATION_RELEASE_GLASS)
			{
				...
				if(this.type == MachineType.CROSS_SEAMER) 
					part.setLastOperation(ComponentOperations.CROSSSEAMER);
				else if(this.type == MachineType.DRILL) 
					part.setLastOperation(ComponentOperations.DRILL);
				else if(this.type == MachineType.GRINDER) 
					part.setLastOperation(ComponentOperations.GRINDER);
				return;
			}
		}
	}
