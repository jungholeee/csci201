	...
	public void checkGlassDetected()
	{
		...
		if (pressed == false)
		{
			...
			for (int k = 0; k < this.activePieces.size(); k++)
			{
				...
				//
				if ((myIndex%2 == 0 && released || !released) && activePieces.get(k).getCenterX()>=getCenterX()-10&&activePieces.get(k).getCenterX()<=getCenterX()+10&&activePieces.get(k).getCenterY()>=getCenterY()-10&&activePieces.get(k).getCenterY()<=getCenterY()+10)
				{
					// End sensor
					setIcon(sensorOn);
					// If one does intersect it will change its icon to pressed(red)
		...
		{
			// If it is already pressed then it will wait until
			// its current glass piece no longer intersects
			// within a certain range and will reset it's icon
			// to red(not pressed).
			if(myIndex%2 == 1 && (currentGlassPiece.getCenterX()<getCenterX()-this.getWidth()/2||currentGlassPiece.getCenterX()>getCenterX()+getWidth()/2||currentGlassPiece.getCenterY()<getCenterY()-getHeight()/2||currentGlassPiece.getCenterY()>getCenterY()+getHeight()/2)) {
				// If completely off the entry sensor
				// Redraws the sensor to its green icon
				setIcon(sensorOff);
			...
			else if(myIndex%2 == 0 && (currentGlassPiece.getCenterX() < getCenterX()-(this.getWidth()/2+currentGlassPiece.getWidth()/2) || currentGlassPiece.getCenterX() > getCenterX()+(this.getWidth()/2+currentGlassPiece.getWidth()/2)
					|| currentGlassPiece.getCenterY() < getCenterY()-(getHeight()/2+currentGlassPiece.getHeight()/2) || currentGlassPiece.getCenterY()>getCenterY()+(getHeight()/2 + currentGlassPiece.getHeight()/2))) {
				// Redraws the sensor to its green icon
				setIcon(sensorOff);
				// Resets the state to false: ie not pressed
				pressed = false;
