...
public class GUIComponentOnline extends GuiAnimationComponent implements ActionListener, Serializable
{
	...
	public void doAnimate()
	{
		...
		else
		{
			...
			// removed because it would cause the piece to auto move off the onlinecomponent
	...
	public void addPart(GUIGlass part)
	{
		...
		transducer.fireEvent(channel, TEvent.WORKSTATION_LOAD_FINISHED, null);
	}
	...
	public void eventFired(TChannel channel, TEvent event, Object[] args)
	{
			...
			;
		{
			...
			{
				// to force the workstation to take the next glass piece
				// working on removing this
				if(args != null)
					this.addPart((GUIGlass)args[0]);
				animationState = AnimationState.MOVING;
			}
			if (event == TEvent.WORKSTATION_RELEASE_GLASS)
			{
				animationState = AnimationState.DONE;
				releasePart = true;
				if(this.type == MachineType.CUTTER)
					this.guiPart.setLastOperation(ComponentOperations.CUTTER);
				else if(this.type == MachineType.OVEN) 
					this.guiPart.setLastOperation(ComponentOperations.OVEN);
				else if(this.type == MachineType.WASHER) 
					this.guiPart.setLastOperation(ComponentOperations.WASHER);
				else if(this.type == MachineType.PAINT) 
					this.guiPart.setLastOperation(ComponentOperations.PAINT);
				else if(this.type == MachineType.UV_LAMP) 
					this.guiPart.setLastOperation(ComponentOperations.UVLAMP);
			}
		}
