...
public class GUIBreakout extends GuiAnimationComponent
{
	...
	{
		this.guiPart = part;
		transducer.fireEvent(TChannel.BREAKOUT, TEvent.WORKSTATION_LOAD_FINISHED, null);
		//***** This was causing the GUI to start animating before it got the agent call!
	...
	private void movePartsOut()
	{
		...
		if (!guiPart.getBounds().intersects(getBounds()))
		{
			...
			/**
			 * @Brooke fixing breakout and manual breakout to match other online components
			 */
			this.transducer.fireEvent(TChannel.BREAKOUT, TEvent.WORKSTATION_RELEASE_FINISHED, null);
			guiPart = null;
	...
	public void eventFired(TChannel channel, TEvent event, Object[] args)
	{
		...
		//This was null whenever an event was fired on the breakout channel
		if (/*channel.toString().equals(this.channel)*/ channel == TChannel.BREAKOUT)
		{
			...
			if (event == TEvent.WORKSTATION_RELEASE_GLASS)
			{
				...
				this.guiPart.setLastOperation(ComponentOperations.BREAKOUT);
			}
		}
