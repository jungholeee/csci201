...
public class GUIConveyor extends GuiComponent
{
	...
	/** GUI HACKS */
	private boolean passGlass = false;
	...
	public void actionPerformed(ActionEvent arg0)
	{
		...
		else if(passGlass) {	// Added as a hack to prevent popup mess-up
			finishPassing();
		}
		for (int i = guiParts.size() - 1; i >= 0; i--)
		{
			...
			//@Brooke added check to make sure not a truck b/c this messes up the truck passing.
			else if((nextComponent.getClass() == GUIPopUp.class || nextComponent.getClass() == GUIShuttle.class) && guiParts.get(i).getBounds().intersects(nextComponent.getBounds())) {
				passGlass = true;
			}
		}
		repaint();
